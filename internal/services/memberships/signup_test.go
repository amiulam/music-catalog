...
func Test_service_SignUp(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantErr: false,
			mockFn: func(args args) {
				mockRepo.EXPECT().GetUser(args.request.Email, args.request.Username, int64(0)).Return(nil, gorm.ErrRecordNotFound)
		...
		},
		{
			...
			wantErr: true,
			mockFn: func(args args) {
				mockRepo.EXPECT().GetUser(args.request.Email, args.request.Username, int64(0)).Return(nil, assert.AnError)
			},
		},
		{
