...
func Test_repository_Get(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantErr: false,
			mockFn: func(args args) {
				mock.ExpectQuery(`SELECT \* FROM "track_activities" .+`).WithArgs(
					args.userID, args.spotifyID, 1,
				).WillReturnRows(
					sqlmock.NewRows([]string{"id", "created_at", "updated_at", "user_id", "spotify_id", "is_liked", "created_by", "updated_by"}).AddRow(1, now, now, 1, "spotifyID", true, "test@gmail.com", "test@gmail.com"),
				)
			},
		},
		{
...
func Test_repository_GetBulkSpotifyIDs(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockFn: func(args args) {
				mock.ExpectQuery(`SELECT \* FROM "track_activities" .+`).WithArgs(
					args.userID, strings.Join(args.spotifyIDs, ","),
				).WillReturnRows(
					sqlmock.NewRows([]string{"id", "created_at", "updated_at", "user_id", "spotify_id", "is_liked", "created_by", "updated_by"}).AddRow(1, now, now, 1, "spotifyID", true, "test@gmail.com", "test@gmail.com"),
				)
			},
